...
class ProviderAdmin(admin.ModelAdmin):
    list_display = ('name', 'slug', 'package_count', 'active_package_count', 'created_at')
    ...
    def sync_provider_packages(self, request, provider_id):
        ...
        if provider.slug == 'esimaccess':
            ...
            messages.success(request, f'{provider.name} paketleri senkronize ediliyor. Task ID: {task.id}')
...
class CountryAdmin(admin.ModelAdmin):
    list_display = ('name', 'code', 'package_count', 'view_esims_button', 'sync_country_button')
    list_filter = ('code',)
    ...
    def view_esims_button(self, obj):
        ...
        return format_html(
            '<a class="button" href="{}">eSIM’leri Görüntüle</a>', url
        )
    ...
    def sync_country_packages(self, request, country_id):
        ...
        messages.success(request, f'{country.name} ({country.code}) paketleri senkronize ediliyor. Task ID: {task.id}')
        return redirect('admin:esim_country_changelist')

@admin.register(eSIMPackage)
class eSIMPackageAdmin(admin.ModelAdmin):
    list_display = (
        'name', 'provider', 'price', 'data_amount_gb', 
        'validity_days', 'country_list', 'is_active', 'updated_at'
    )
    list_filter = (
        'provider', 'is_active', 'validity_days', 
        PriceRangeFilter,
    ...
    def sync_all_view(self, request):
        if request.method == 'POST':
            task = sync_all_esim_packages.delay()
            messages.success(request, f'Tüm paket senkronizasyonu başlatıldı. Task ID: {task.id}')
            return redirect('admin:esim_esimpackage_changelist')
        
        return render(request, 'admin/esim/sync_all.html', {
    ...
    def sync_country_view(self, request):
        if request.method == 'POST':
            ...
            if country_code:
                ...
                messages.success(request, f'{country_code} ülkesi {action} başlatıldı. Task ID: {task.id}')
                return redirect('admin:esim_esimpackage_changelist')
        
        countries = Country.objects.all().order_by('name')
        return render(request, 'admin/esim/sync_country.html', {
    ...
    def cleanup_view(self, request):
        if request.method == 'POST':
            ...
            if confirm:
                ...
                messages.success(request, f'{days} gün önceki paketler temizleniyor. Task ID: {task.id}')
                return redirect('admin:esim_esimpackage_changelist')
        ...
        return render(request, 'admin/esim/cleanup.html', {
    ...
    def validate_view(self, request):
        if request.method == 'POST':
            ...
            messages.success(request, f'Veri doğrulaması başlatıldı. Task ID: {task.id}')
            return redirect('admin:esim_esimpackage_changelist')
        
        return render(request, 'admin/esim/validate.html', {
    ...
    def stats_view(self, request):
        ...
        for provider in Provider.objects.all():
            ...
            active = eSIMPackage.objects.filter(provider=provider, is_active=True).count()
        ...
        return render(request, 'admin/esim/stats.html', {
