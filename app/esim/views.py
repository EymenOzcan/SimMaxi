...
def sync_country_packages(request):
    ...
    if not country_code:
        return Response({
            'status': 'error',
            'message': 'country_code parametresi gerekli'
...
def batch_sync_countries_view(request):
    ...
    if not country_codes or not isinstance(country_codes, list):
        return Response({
            'status': 'error',
            'message': 'country_codes listesi gerekli'
        }, status=status.HTTP_400_BAD_REQUEST)
...
def cleanup_old_packages_view(request):
    ...
    try:
        ...
        return Response({
            'status': 'error',
            'message': 'days parametresi geçerli bir sayı olmalı'
        }, status=status.HTTP_400_BAD_REQUEST)
    except Exception as e:
        return Response({
            'status': 'error',
...
def get_package_stats(request):
    ...
    try:
        ...
        for provider in Provider.objects.all():
            ...
            active_packages = eSIMPackage.objects.filter(provider=provider, is_active=True).count()
    ...
    try:
        ...
        if provider == 'all':
            ...
            countries = Country.objects.filter(
                esimpackage__provider__slug=provider,
                esimpackage__is_active=True
...
def search_packages(request):
    ...
    try:
        ...
        min_data = request.GET.get('min_data')  # MB cinsinden
        max_data = request.GET.get('max_data')  # MB cinsinden
        min_validity = request.GET.get('min_validity')  # gün cinsinden
        max_validity = request.GET.get('max_validity')  # gün cinsinden
        ...
        for pkg in packages:
            ...
            package_data.append({
                ...
                'data_amount_gb': round(pkg.data_amount_mb / 1024, 2) if pkg.data_amount_mb != -1 else 'Unlimited',
