...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--dry-run',
            action='store_true',
            help='Sadece göster, silme işlemi yapma'
        )
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(f'🔍 DRY RUN: {count} paket silinecek (son {days} gün içinde güncellenmemiş pasif paketler)')
            )
            
            # İlk 10 paketi göster
            for pkg in packages_to_delete[:10]:
                self.stdout.write(f'  - {pkg.name} ({pkg.provider.name}) - Son güncelleme: {pkg.updated_at}')
