...
def sync_country_esim_packages(self, country_code):
    ...
    try:
        logger.info(f"{country_code} ülkesi eSIM paketleri senkronizasyonu başlatıldı")
        service = eSIMService()
        service.sync_country_packages(country_code)
        logger.info(f"{country_code} ülkesi eSIM paketleri başarıyla senkronize edildi")
        return {"status": "success", "message": f"{country_code} paketleri senkronize edildi"}
    except Exception as exc:
...
def sync_esimaccess_packages():
    ...
    try:
        ...
        return {"status": "success", "message": "eSIM Access paketleri senkronize edildi"}
    except Exception as exc:
...
def batch_sync_countries(country_codes):
    ...
    for country_code in country_codes:
        try:
            ...
            results.append({"country": country_code, "status": "error", "message": str(exc)})
    
    success_count = len([r for r in results if r["status"] == "success"])
    error_count = len([r for r in results if r["status"] == "error"])
    
    logger.info(f"Toplu senkronizasyon tamamlandı. Başarılı: {success_count}, Hatalı: {error_count}")
...
def batch_update_countries(country_codes):
    ...
    logger.info(f"Toplu güncelleme tamamlandı. Başarılı: {success_count}, Hatalı: {error_count}")
...
def validate_package_data():
    ...
    try:
        ...
        success_rate = ((total_active - problematic_count) / total_active * 100) if total_active > 0 else 0
        
        logger.info(f"Veri doğrulaması tamamlandı. {problematic_count}/{total_active} pakette sorun bulundu")
